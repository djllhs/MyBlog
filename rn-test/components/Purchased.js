...
class Course extends Component {
    constructor(props) {
        ...
        const ds = new ListView.DataSource({rowHasChanged: (r1, r2) => r1 !== r2});
        this.state = {
            dataSource: ds.cloneWithRows(['1dasda是范德萨发大幅度反弹联发科的酸辣粉尽快提呕吐就交给你', '2',3 ])
        };
        this._renderRow = this._renderRow.bind(this);
    }

    _renderRow(rowData) {
        const {mainItemWrap, mainItem, mainItemText, courseItemWrap, courseItem} = purchasedStyle;
        console.log(rowData);
        return (
            <View style={[mainItemWrap, courseItemWrap]}>
                <View style={[mainItem, courseItem]}>
                    <Image style={courseItem} source={require("../img/ic_course_logo_default.png")}/>
                </View>
                <Text numberOfLines={2} style={mainItemText}>{rowData}</Text>
            </View>
        )
    ...
    render() {
        ...
        return this.state.dataSource.length >= 0 ? (
         ...
         (
             <ListView contentContainerStyle={[main, courseMain]}
                    dataSource={this.state.dataSource}
                    renderRow={ this._renderRow}
             />
